...
struct Application
{
	static cxxopts::Options getOptions()
	{
		...
		options.add_options()
			("w,width", "Width of the window", cxxopts::value<uint16_t>()->default_value("1280"))
			("h,help", "Show help")
		;
		return options;
	}
	...
	Application(int argc, char **argv, cxxopts::ParseResult inputArgs) :
		...
		m_AppPath{ c2ba::fs::path{ argv[0] } },
		m_AppName{ m_AppPath.stem().string() },
		m_ImGuiIniFilename{ m_AppName + ".imgui.ini" },
		m_ShadersRootPath{ m_AppPath.parent_path() / "shaders" },
		m_nWindowWidth{ m_InputArgs["width"].as<uint16_t>() },
		m_nWindowHeight{ (uint16_t)(m_nWindowWidth * 9.f / 16) }
	{
